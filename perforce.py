...
class FinishedP4Operation(Exception):
    pass


class TimedOutP4Operation(Exception):
    pass
...
class P4Output(dict):
    ...
    def __init__(self, outStr, keysColonDelimited=False):
        ...
        if isinstance(outStr, str):
            ...
            print(outStr)
            raise P4Exception(
                "unsupported type (%s) given to %s" %
                (type(outStr), self.__class__.__name__))
        ...
        for line in lines:
            ...
            if keysColonDelimited:
                prefix = ''.join([(s, s.capitalize())[n]
                                  for n, s in enumerate(prefix.lower().split())])
            else:
                prefix = prefix[0].lower() + prefix[1:]

            self[prefix] = data

        # finally, if there are prefixes which have a numeral at the end, strip
        # it and pack the data into a list
        multiKeys = {}
        for k in self.keys():
...
class P4Change(dict):
    ...
    def FetchByNumber(cls, number):
        ...
        if change == 0:
            ...
            info = p4Info()
            fileInfo = _p4run(
                'opened -u %s -C %s -c default' %
                (info.userName, info.clientName))
    ...
    def FetchByDescription(cls, description, createIfNotFound=False):
        ...
        '''
        cleanDesc = ''.join([s.strip()
                             for s in description.lower().strip().split('\n')])
        for change in cls.IterPending():
            thisDesc = ''.join(
                [s.strip() for s in change.description.lower().strip().split('\n')])
            ...
            try:
                while True:
                    ...
                    if line.startswith('Change'):
                        ...
                        curChange.date = datetime.date(
                            *list(map(int, toks[3].split('/'))))
                        curChange.description = ''

                        # setup triggers for other data in the changelist that
                        # doesn't get returned by the changes command - see the
                        # __getattr__ doc for more info
    ...
    def IterPending(cls):
        ...
        info = p4Info()
        changes = _p4run(
            'changes -u %s -s pending -c %s' %
            (info.userName, info.clientName))

        if changes[0].startswith('error:'):
            raise P4Exception(
                'Could not retreive changelist data from Perforce server!\n{0}'.format(
                    changes[0]))
...
# all opened perforce files get added to a changelist with this
# description by default
DEFAULT_CHANGE = 'Tools Auto-Checkout'

# gets called when a perforce command takes too long (defined by
# path.P4File.TIMEOUT_PERIOD)
...
def lsP4(queryStr, includeDeleted=False):
    ...
    for line in queryLines:
        ...
        # deal with error lines, or exit lines (an exit prefix may not actually
        # mean the end of the data - the query may have been broken into
        # batches)
...
def toDepotAndDiskPaths(files):
    ...
    for filesChunk in iterBy(files, 15):
        # last line is the "exit" line...
        lines += _p4run('-ztag where', *filesChunk)[:-1]
...
def findRedundantPYCs(rootDir=None, recursive=True):
    ...
    if rootDir.exists:
        ...
        for f in bytecodeFiles:
            ...
            # is there a corresponding py script for this file?  if it does,
            # the pyc is safe to delete - so delete it
            if pyF.exists:
